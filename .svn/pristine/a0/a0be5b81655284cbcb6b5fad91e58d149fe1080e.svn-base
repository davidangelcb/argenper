...
function listSinFechaEntrega($_GET)
{
	...
	$dataTotal = DbArgenper::fetchOne($query);//
        ...
        foreach($giros as $row){
            ...
            $response->rows[$i]['cell']=array('',$row['dias'],$row['numero_giro'],htmlentities($row['nombre_cliente']),$row['celular_cliente'],$row['mensaje_cliente'],$row['fecha_proceso'],$estado,$row['numero_envios']); 
            $i++; 
        }
