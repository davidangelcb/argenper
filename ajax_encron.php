...
function listPen($_GET)
{
        ...
        $query = "SELECT COUNT(*) AS count from  argenper_tblSMS where estado_envio in (6)  ".$filtro.$dateFilter."";
	$dataTotal = DbArgenper::fetchOne($query);//
        ...
        $SQL = "SELECT  id_ref as id,tipo,tip_entrega, nombre_oficina as oficina,DATE_FORMAT(fecha_proceso,'%m-%d-%Y') as fecha_proceso ,estado_envio, numero_giro,  nombre_cliente,  respuesta_api, celular_cliente,  mensaje_cliente FROM   argenper_tblSMS where estado_envio  in (6)  ".$filtro.$dateFilter."   ORDER BY $sidx $sord LIMIT $start , $limit"; 
        ...
        foreach($giros as $row){
            ...
            $response->rows[$i]['cell']=array('',$row['numero_giro'],$row['oficina'],htmlentities($row['nombre_cliente']),$row['fecha_proceso'],$row['respuesta_api'],$row['celular_cliente'],$row['mensaje_cliente']); 
            $i++; 
        }
